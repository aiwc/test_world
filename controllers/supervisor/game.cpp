...
std::string random_string(std::size_t len) {
  ...
  constexpr std::size_t alphanum_size =
      sizeof(alphanum) - 1; // -1 to exclude terminating null
...
MSGPACK_API_VERSION_NAMESPACE(MSGPACK_DEFAULT_API_NS) {
  ...
  template <> struct object_with_zone<subimage> {
    void operator()(msgpack::object::with_zone &o, const subimage &v) const {
      ...
      o.via.map.ptr[0] = {msgpack::object("x", o.zone),
                          msgpack::object(v.x, o.zone)};
      o.via.map.ptr[1] = {msgpack::object("y", o.zone),
                          msgpack::object(v.y, o.zone)};
      o.via.map.ptr[2] = {msgpack::object("w", o.zone),
                          msgpack::object(v.w, o.zone)};
      o.via.map.ptr[3] = {msgpack::object("h", o.zone),
                          msgpack::object(v.h, o.zone)};
      o.via.map.ptr[4] = {msgpack::object("base64", o.zone),
                          msgpack::object(v.base64, o.zone)};
    }
  };
...
game::game(supervisor &sv, std::size_t rs_port, std::string uds_path)
    : sv_(sv), rs_port_(rs_port), uds_path_(uds_path) {}
...
void game::run() {
  ...
  if (!config_file)
    throw std::runtime_error(
        "Could not read 'config.json' configuration file.");

  std::string buffer((std::istreambuf_iterator<char>(config_file)),
  ...
  // gets game rules from 'config.json' (if no rules specified, default options
  // are given)
  {
    game_time_ms_ = c::DEFAULT_GAME_TIME_MS / c::PERIOD_MS * c::PERIOD_MS;
    deadlock_flag_ = true;

    if (config_json.HasMember("rule") &&
        config_json["rule"].IsObject()) { // set rules
      if (config_json["rule"].HasMember("game_time") &&
          config_json["rule"]["game_time"].IsNumber())
        game_time_ms_ =
            static_cast<size_t>(config_json["rule"]["game_time"].GetDouble() *
                                1000) /
            c::PERIOD_MS * c::PERIOD_MS;

      if (config_json["rule"].HasMember("deadlock") &&
          config_json["rule"]["deadlock"].IsBool())
        deadlock_flag_ = config_json["rule"]["deadlock"].GetBool();
    } else
      std::cout << "\"rule\" section of 'config.json' seems to be missing: "
                   "using default options"
                << std::endl;

    std::cout << "Rules:" << std::endl;
    std::cout << "     game duration - " << game_time_ms_ / 1000.0 << " seconds"
              << std::endl;
    std::cout << "          deadlock - " << (deadlock_flag_ ? "on" : "off")
              << std::endl;
  }

  // gets other options from 'config.json' (if no option is specified, default
  // option is given)
  {
    ...
    if (config_json.HasMember("tool") &&
        config_json["tool"].IsObject()) { // set other options if specified
      if (config_json["tool"].HasMember("repeat") &&
          config_json["tool"]["repeat"].IsBool())
        repeat = config_json["tool"]["repeat"].GetBool();

      // if repeat is enabled, record is forced to be disabled
      if (repeat)
        std::cout << "Game repetition is enabled that the game recording will "
                     "be disabled."
                  << std::endl;
      else {
        if (config_json["tool"].HasMember("record") &&
            config_json["tool"]["record"].IsBool())
          record = config_json["tool"]["record"].GetBool();

        if (record && config_json["tool"].HasMember("record_path") &&
            config_json["tool"]["record_path"].IsString())
  ...
  for (const auto &team : {T_RED, T_BLUE}) {
    ...
    // my team
    const std::string &name = ((config_json[tc].HasMember("name") &&
                                config_json[tc]["name"].IsString())
                                   ? config_json[tc]["name"].GetString()
                                   : "");
    const double &rating = 0; // rating is disabled
    const std::string &exe = ((config_json[tc].HasMember("executable") &&
                               config_json[tc]["executable"].IsString())
                                  ? config_json[tc]["executable"].GetString()
                                  : "");
    const std::string &data = ((config_json[tc].HasMember("datapath") &&
                                config_json[tc]["datapath"].IsString())
                                   ? config_json[tc]["datapath"].GetString()
                                   : "");

    // opponent
    const std::string &name_op = ((config_json[tc_op].HasMember("name") &&
                                   config_json[tc_op]["name"].IsString())
                                      ? config_json[tc_op]["name"].GetString()
                                      : "");
    const double &rating_op = 0; // rating is disabled

    const auto ret = player_team_infos_.emplace(
        std::piecewise_construct, std::make_tuple(random_string(c::KEY_LENGTH)),
    ...
    map info;
    info.emplace_back("field", msgpack::object(std::make_tuple(c::FIELD_LENGTH,
                                                               c::FIELD_WIDTH),
                                               z_info_));
    info.emplace_back(
        "goal", msgpack::object(std::make_tuple(c::GOAL_DEPTH, c::GOAL_WIDTH),
                                z_info_));
    info.emplace_back("penalty_area",
                      msgpack::object(std::make_tuple(c::PENALTY_AREA_DEPTH,
                                                      c::PENALTY_AREA_WIDTH),
                                      z_info_));
    info.emplace_back(
        "goal_area",
        msgpack::object(std::make_tuple(c::GOAL_AREA_DEPTH, c::GOAL_AREA_WIDTH),
                        z_info_));
    ...
    info.emplace_back("robot_height",
                      msgpack::object(c::ROBOT_HEIGHT, z_info_));
    info.emplace_back("axle_length", msgpack::object(c::AXLE_LENGTH, z_info_));
    info.emplace_back("robot_body_mass",
                      msgpack::object(c::ROBOT_BODY_MASS, z_info_));

    info.emplace_back("wheel_radius",
                      msgpack::object(c::WHEEL_RADIUS, z_info_));
    info.emplace_back("wheel_mass", msgpack::object(c::WHEEL_MASS, z_info_));

    info.emplace_back("max_linear_velocity",
                      msgpack::object(c::MAX_LINEAR_VELOCITY, z_info_));
    info.emplace_back("max_torque", msgpack::object(c::MAX_TORQUE, z_info_));

    info.emplace_back(
        "resolution",
        msgpack::object(std::make_tuple(c::RESOLUTION_X, c::RESOLUTION_Y),
                        z_info_));
    info.emplace_back("number_of_robots",
                      msgpack::object(c::NUMBER_OF_ROBOTS, z_info_));
    info.emplace_back("codewords", msgpack::object(c::CODEWORDS, z_info_));
    info.emplace_back("game_time",
                      msgpack::object(game_time_ms_ / 1000., z_info_));

    info.emplace_back(
        "team_info",
        msgpack::object(
            std::make_tuple(
                map{std::make_pair("name", msgpack::object(name, z_info_)),
                    /* */ std::make_pair("rating",
                                         msgpack::object(rating, z_info_))},
                /* */ map{std::make_pair("name",
                                         msgpack::object(name_op, z_info_)),
                          /* */ std::make_pair(
                              "rating", msgpack::object(rating_op, z_info_))}),
            /* */ z_info_));
  ...
  if (config_json.HasMember("commentator") &&
      config_json["commentator"].IsObject()) {
    const std::string &name =
        ((config_json["commentator"].HasMember("name") &&
          config_json["commentator"]["name"].IsString())
             ? config_json["commentator"]["name"].GetString()
             : "");
    const std::string &exe =
        ((config_json["commentator"].HasMember("executable") &&
          config_json["commentator"]["executable"].IsString())
             ? config_json["commentator"]["executable"].GetString()
             : "");
    const std::string &data =
        ((config_json["commentator"].HasMember("datapath") &&
          config_json["commentator"]["datapath"].IsString())
             ? config_json["commentator"]["datapath"].GetString()
             : "");
    ...
    if (!exe.empty()) {
      ...
      std::cout << "Commentator \"executable\" is missing: skipping commentator"
                << std::endl;
  } else
    std::cout << "\"commentator\" section of 'config.json' seems to be "
                 "missing: skipping commentator"
              << std::endl;
  ...
  if (config_json.HasMember("reporter") && config_json["reporter"].IsObject()) {
    const std::string &name = ((config_json["reporter"].HasMember("name") &&
                                config_json["reporter"]["name"].IsString())
                                   ? config_json["reporter"]["name"].GetString()
                                   : "");
    const std::string &exe =
        ((config_json["reporter"].HasMember("executable") &&
          config_json["reporter"]["executable"].IsString())
             ? config_json["reporter"]["executable"].GetString()
             : "");
    const std::string &data =
        ((config_json["reporter"].HasMember("datapath") &&
          config_json["reporter"]["datapath"].IsString())
             ? config_json["reporter"]["datapath"].GetString()
             : "");
    ...
    if (!exe.empty()) {
      ...
      std::cout << "Reporter \"executable\" is missing: skipping reporter"
                << std::endl;
  } else
    std::cout << "\"reporter\" section of 'config.json' seems to be missing: "
                 "skipping reporter"
              << std::endl;
  ...
  // run or finish the game
  {
    const auto count = std::count_if(
        std::cbegin(player_team_infos_), std::cend(player_team_infos_),
        [](const auto &kv) {
          return kv.second.role == ROLE_PLAYER && kv.second.is_ready == true;
        });
    if (count == 0) {
      ...
      try {
        ...
        if (record) {
          ...
          record_fullpath = record_path + "/[" +
                            std::to_string(timeinfo->tm_year + 1900) + "-" +
                            std::to_string(timeinfo->tm_mon + 1) + "-" +
                            std::to_string(timeinfo->tm_mday) + "T" +
                            std::to_string(timeinfo->tm_hour) + "_" +
                            std::to_string(timeinfo->tm_min) + "_" +
                            std::to_string(timeinfo->tm_sec) + "]" +
                            team_name[T_RED] + "_" + team_name[T_BLUE] + ".mp4";
          sv_.movieStartRecording(record_fullpath, 1920, 1080, 0, 100, 1,
                                  false);
        }
        ...
        const auto until = std::chrono::steady_clock::now() +
                           std::chrono::milliseconds(c::WAIT_KILL_MS);
        ...
        for (auto &kv : player_team_infos_) {
          ...
          // boost 1.65 or lower has a bug in child::wait_until(). use
          // child::wait_for().
        ...
        if (record) {
          // Stop game recording
          std::cout << "Saving the recorded game as: " << record_fullpath
                    << std::endl;
          std::cout << "Please wait until the message \033[36m\"INFO: Video "
                       "creation finished.\"\033[0m is shown."
                    << std::endl;
  ...
  if (report.size() > 0) {
    std::ofstream rfile(std::string("../../reports/") +
                        config_json["reporter"]["name"].GetString() + ".txt");
...
void game::connect_to_server() {
  ...
  transport_ =
      std::make_shared<autobahn::wamp_uds_transport>(io_, uds_endpoint);
#else
  boost::asio::ip::tcp::endpoint tcp_endpoint(
      boost::asio::ip::address::from_string(c::SERVER_IP), rs_port_);
  transport_ =
      std::make_shared<autobahn::wamp_tcp_transport>(io_, tcp_endpoint);
#endif
  session_ = std::make_shared<autobahn::wamp_session>(io_);
  transport_->attach(
      std::static_pointer_cast<autobahn::wamp_transport_handler>(session_));
  ...
  session_
      ->provide("aiwc.bootup",
                [&](autobahn::wamp_invocation i) { return on_bootup(i); })
      .get();
  session_
      ->provide("aiwc.ready",
                [&](autobahn::wamp_invocation i) { return on_ready(i); })
      .get();
  session_
      ->provide("aiwc.get_info",
                [&](autobahn::wamp_invocation i) { return on_info(i); })
      .get();
  session_
      ->provide("aiwc.set_speed",
                [&](autobahn::wamp_invocation i) { return on_set_speed(i); })
      .get();
  session_
      ->provide("aiwc.commentate",
                [&](autobahn::wamp_invocation i) { return on_commentate(i); })
      .get();
  session_
      ->provide("aiwc.report",
                [&](autobahn::wamp_invocation i) { return on_report(i); })
      .get();
}
...
void game::run_participant() {
  ...
  for (const auto &kv : player_team_infos_) {
    bootup_futures.emplace_back(
        session_->call("aiwc.bootup", std::make_tuple(kv.first)));
  }
  ...
  if (getenv("WEBOTS_FIREJAIL_CONTROLLERS") != NULL) {
    ...
    firejail_command =
        "firejail --quiet --shell=none --nosound --net=br0 --netfilter=" +
        boost::filesystem::absolute("player.net").string() + " ";
  }
#endif
  ...
  try {
    for (auto &kv : player_team_infos_) {
      ...
      if (ti.executable.compare(ti.executable.length() - 3, 3, ".py") ||
          !boost::filesystem::exists(ti.executable)) {
#endif
        ti.c = bp::child(
            bp::exe = p_exe.filename().string(),
            bp::args = {c::SERVER_IP, std::to_string(rs_port_), c::REALM, key,
                        boost::filesystem::absolute(ti.datapath).string()},
            bp::start_dir = p_exe.parent_path());
#ifdef _WIN32
      } else { // if python script, enter special handler
        ti.c = bp::child(
            bp::exe = bp::search_path("python").string(),
            bp::args = {p_exe.filename().string(), c::SERVER_IP,
                        std::to_string(rs_port_), c::REALM, key,
                        boost::filesystem::absolute(ti.datapath).string()},
            bp::start_dir = p_exe.parent_path());
      }
#endif
    ...
    throw std::runtime_error(
        "one of the given executables does not exist, or cannot run");
  }
}
...
void game::update_label() {
  if (half_passed_ == false) {
    ...
    sv_.setLabel(0,
                 (boost::format("score %d:%d, time %.2f") % score_[0] %
                  score_[1] % (time_ms_ / 1000.))
                     .str(),
                 0.4, 0.95,        // x, y
    ...
    sv_.setLabel(0,
                 (boost::format("score %d:%d, time %.2f") % score_[1] %
                  score_[0] % ((game_time_ms_ + time_ms_) / 1000.))
                     .str(),
                 0.4, 0.95,        // x, y
                 0.10, 0x00000000, // size, color
                 0, "Arial"        // transparency, font
  ...
  for (std::size_t i = 0; i < comments_.size(); ++i) {
    sv_.setLabel(comments_start + i, comments_[i], 0.01,
                 0.01 + 0.04 * i,  // x, y
...
void game::step(std::size_t ms) {
  // we assume that world.basicTimeStep doesn't change and the given 'ms' is
  // multiple of the basic time step
  const std::size_t basic_time_step =
      static_cast<std::size_t>(sv_.getBasicTimeStep());
...
void game::pause() { paused_.store(true); }
...
void game::resume() { paused_.store(false); }
  ...
  for (const auto &team : {T_RED, T_BLUE}) {
    for (std::size_t id = 0; id < c::NUMBER_OF_ROBOTS; ++id) {
      if (activeness_[team][id]) {
        ...
        speed[0] = std::max(std::min(speed[0], c::MAX_LINEAR_VELOCITY[id]),
                            -c::MAX_LINEAR_VELOCITY[id]);
        speed[1] = std::max(std::min(speed[1], c::MAX_LINEAR_VELOCITY[id]),
                            -c::MAX_LINEAR_VELOCITY[id]);
...
bool game::get_corner_ownership() {
  ...
  for (const auto &team : {T_RED, T_BLUE}) {
    for (std::size_t id = 0; id < c::NUMBER_OF_ROBOTS; ++id) {
      ...
      if ((s_x * x > c::FIELD_LENGTH / 2 - c::PENALTY_AREA_DEPTH) &&
          (s_y * y > c::PENALTY_AREA_WIDTH / 2)) {
        const auto distance_squared =
            (x - ball_x) * (x - ball_x) + (y - ball_y) * (y - ball_y);
        robot_count[team] += 1;
  ...
  // tie breaker - team with robots (within the decision region) closer to the
  // ball on average gets the ownership
  else {
...
bool game::check_penalty_area() {
  ...
  if ((std::abs(ball_x) < c::FIELD_LENGTH / 2 - c::PENALTY_AREA_DEPTH) ||
      (std::abs(ball_y) > c::PENALTY_AREA_WIDTH / 2)) {
    return false;
  }
...
bool game::ball_in_field() {
  ...
  if ((abs_x > c::FIELD_LENGTH / 2 + c::WALL_THICKNESS) &&
      (abs_y > c::GOAL_WIDTH / 2 + c::WALL_THICKNESS))
    return false;
...
bool game::any_object_nearby(double target_x, double target_y,
                             double target_r) {
  // check ball position
  const auto pos = sv_.get_ball_position();
...
void game::publish_current_frame(std::size_t reset_reason) {
  ...
  std::array<std::array<std::tuple<double, double, double, bool, bool>,
                        c::NUMBER_OF_ROBOTS>,
             2>
      g_robots;
  for (const auto &team : {T_RED, T_BLUE}) {
...

  {
    for (auto &kv : player_team_infos_) {
      ...
      if (!ti.is_red) { // blue team sees red as blue, blue as red
        ...
        if (reason == c::SCORE_RED_TEAM)
          reason = c::SCORE_BLUE_TEAM;
        else if (reason == c::SCORE_BLUE_TEAM)
          reason = c::SCORE_RED_TEAM;
      }

      msgpack::type::assoc_vector<std::string, msgpack::object> msg;
      msgpack::zone z;

      // The first frame contains time, score, reset_reason, game_state, and
      // ball ownership
      ...
      if (game_state != c::STATE_DEFAULT)
        msg.emplace_back("ball_ownership",
                         msgpack::object(ti.is_red ? ball_ownership_ == T_RED
                                                   : ball_ownership_ == T_BLUE,
                                         z));
      else
      ...
      constexpr std::size_t n_max_subimages =
          c::MSG_MAX_SIZE / c::ESTIMATED_SUBIMAGE_SIZE;
          ...
          if (subimages.size() > n_max_subimages) {
            ...
            std::copy(std::make_move_iterator(subimages.begin()),
                      std::make_move_iterator(
                          std::next(subimages.begin(), n_max_subimages)),
            ...
            events.emplace_back(topic, msgpack::object(std::make_tuple(msg), z),
                                std::move(z));
            msg.clear();
            z = msgpack::zone{};
          } else { // last msg
      ...
      msg.emplace_back(
          "coordinates",
          msgpack::object(std::make_tuple(robots[T_RED], robots[T_BLUE], ball),
                          z));
  ...
  std::copy(std::make_move_iterator(events.begin()),
            std::make_move_iterator(events.end()), std::back_inserter(events_));
  lck_events.unlock();
  events_cv_.notify_one();
}
...
void game::run_game() {
  ...
  for (;;) {
    ...
    for (auto &team_touch : touch_) {
      for (auto &robot_touch : team_touch) {
        // if any of the robots has touched the ball at this frame, update
        // recent_touch_
        if (robot_touch) {
          recent_touch_ = touch_;
        }
    ...
    // check if any of robots has fallen
    {
      for (const auto &team : {T_RED, T_BLUE}) {
        for (std::size_t id = 0; id < c::NUMBER_OF_ROBOTS; id++) {
          // if a robot has fallen and could not recover for c::FALL_TIME_MS,
          // send the robot to foulzone
          if (activeness_[team][id] &&
              time_ms_ - fall_time_[team][id] >= c::FALL_TIME_MS) {
            activeness_[team][id] = false;
    ...
    case c::STATE_DEFAULT: {
      ...
      if ((std::abs(ball_x) > c::FIELD_LENGTH / 2) &&
        ...
        reset(((ball_ownership_ == T_RED) ? c::FORMATION_KICKOFF
                                          : c::FORMATION_DEFAULT),
              ((ball_ownership_ == T_BLUE) ? c::FORMATION_KICKOFF
                                           : c::FORMATION_DEFAULT));
      ...
      // check if any of robots should return to the field
      {
        for (const auto &team : {T_RED, T_BLUE}) {
          for (std::size_t id = 0; id < c::NUMBER_OF_ROBOTS; id++) {
            // sentout time of 0 is an indicator that the robot is currently on
            // the field
            if (sentout_time_[team][id] == 0)
              continue;
            // if a robot has been sent out and c::SENTOUT_DURATION_MS has
            // passed, return the robot back to the field
            if (time_ms_ - sentout_time_[team][id] >= c::SENTOUT_DURATION_MS) {
              // if any object is located within 1.5 * robot_size, the return is
              // delayed
              const auto s = (team == T_RED) ? 1 : -1;
              const auto x =
                  c::ROBOT_FORMATION[c::FORMATION_DEFAULT][id][0] * s;
              const auto y =
      ...
      if (reset_reason == c::NONE && deadlock_flag_ == true) {
        ...
        // if the ball is not moved fast enough for c::DEADLOCK_DURATION_MS
          ...
          // if the deadlock happened in the general region, relocate the ball
    ...
    case c::STATE_KICKOFF: {
      ...
      if (time_ms_ - kickoff_time_ >= c::KICKOFF_TIME_LIMIT_MS) {
        ...
        if (ball_x * ball_x + ball_y * ball_y >
            c::KICKOFF_BORDER * c::KICKOFF_BORDER) {
          unlock_all_robots();
          game_state_ = c::STATE_DEFAULT;
        }
      }
...
void game::on_bootup(autobahn::wamp_invocation invocation) {
  ...
  if (std::all_of(std::cbegin(player_team_infos_),
                  std::cend(player_team_infos_),
...
// called from io thread. it just sets the wheel speed and not transfer it to
// the simulator.
...
void game::on_commentate(autobahn::wamp_invocation invocation) {
  ...
  std::unique_lock<std::mutex> lck(m_comments_);
  if (half_passed_ == false)
    comments_.push_back((boost::format("[%.2f] ") % (time_ms_ / 1000.)).str() +
                        invocation->argument<std::string>(1));
  else
    comments_.push_back(
        (boost::format("[%.2f] ") % ((game_time_ms_ + time_ms_) / 1000.))
            .str() +
        invocation->argument<std::string>(1));
